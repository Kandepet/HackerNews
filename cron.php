...
$c_config = new HackerNews\Config('config.ini');
$base_url = $c_config['site.base_url'] . $c_config['site.base_path'] . '/';
...
//update_setting('cron_time',time(), $db);

// Get all stories in the last 24hrs.
$db->where("story_time", (time()-86400), ">");
$stories = $db->get("stories", 600, Array("story_id", "story_time", "story_votes", "story_title", "story_buries", "story_prom"));
...
foreach($stories as $story) {
   ...
   $db->update("stories", Array("story_rating" => HackerNews\Common::validate_input($score)));
